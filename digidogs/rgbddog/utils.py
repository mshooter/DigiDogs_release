...
from digidogs.rgbddog import reader as reader
...
'''	
def ReadBvhFile(filname, onTheSpot=False, scale=1, transposeAfterReading=True, gta=False):
    readInst = reader.MyReader(filname, onTheSpot, scale, gta=gta)
    ...
    if transposeAfterReading:
    	...
    	for node in nodes:
    		...
    		for i in range(numFrames):
    			r = Rsci.from_matrix(node.rotation_local[i,:,:])
...
def GetLocalRotationsForFrame(nodes, frame, asType='quat', reorder=True):
	...
	if asType == 'quat':
		...
		for node in nodes:
			...
			r = Rsci.from_matrix(rot_local[frame]).as_quat()
		...
		for node in nodes:
			rot_local = np.array(node.rotation_local)
			r = Rsci.from_matrix(rot_local[frame]).as_rotvec()
			rot[node.idx] = r[order]
	else:
...
def GetWorldRotationsForFrame(nodes, frame, asType='quat', reorder=True):
	...
	if asType == 'quat':
		...
		for node in nodes:
			...
			r = Rsci.from_matrix(rot_world[frame]).as_quat()
			rot[node.idx] = r[order]
	elif asType == 'rodrigues':
		rot = np.empty((numJoints,3))
		order = np.array((0,1,2))
		...
		for node in nodes:
			rot_world = np.array(node.rotation_world)
			r = Rsci.from_matrix(rot_world[frame]).as_rotvec()
			rot[node.idx] = r[order]
	else:
		rot = np.empty((numJoints,3,3))
		for node in nodes:
